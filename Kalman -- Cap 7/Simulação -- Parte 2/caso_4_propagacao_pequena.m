...
for k = 1:max(size(z))
    ...
    % =====================================================================
    % Realiza propagação do estado para o próximo ponto de amostragem.
    interval  = 0.001;      % Intervalo para integração.
    X_hat0    = X_hat{k};
    for i = 1:((Ts/interval))
        ti(i) = (i - 1)*interval;
        
        c1     = f(X_hat0, ti(i));
        c2     = f(X_hat0, ti(i) + interval); 
        
        X_hat0 = X_hat0 + (1/2)*interval*(c1 + c2);
    end
    X_hat{k+1} = X_hat0;
    % =====================================================================
